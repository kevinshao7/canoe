...
static Real p0, Rd, cp, Ts, Rp, grav, eq_heat_flux,sday,syear, pi, dday, Omega, sigma, emissivity,adtdz,cv,spinupflux,flux_ratio,Tint;
Real current_time;
...
Real Insolation(MeshBlock *pmb,int k,int j,Real time){
  ...
  if (cosomega>0){
    if (time > 5.E9){
      ...
      Real fluxmax = eq_heat_flux + spinupflux/exp( time /5.E8);
...
void Forcing(MeshBlock *pmb, Real const time, Real const dt,
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)
      for (int j = pmb->js; j <= pmb->je; ++j) {
        ...
        du(IEN, k, j, pmb->is) += 0.25*0.14*fluxmax*dt/dz; //internal heat flux
        for (int i = pmb->is; i<= pmb->ie; ++i){
          Real press = pmb->phydro->w(IPR, k, j,i);
          if (press <= 1e5){
            Real dz = pmb->pcoord->dx1f(i);
            du(IEN, k, j, i) += 0.90*Insolation(pmb,k,j,time) * dt / dz;
            break;
          }
        }         
        //cooling layer 1 
        for (int i = pmb->is; i<= pmb->ie; ++i){
          ...
          if (press <= 1e4){
            ...
            Real deltatemp = pow((3*flux_ratio*sigma*dt/(cv*dz)+(1/_cube(temp))),-(1.0/3.0)) - temp;
            du(IEN, k, j, i) += cv*deltatemp;
            // du(IEN, k, j, i) -= dt*cool_coeff*sigma*_qur(temp)/2;

            // dz = pmb->pcoord->dx1f(i+1);
            // cool_coeff = flux_ratio/dz;
            // temp = pmb->phydro->w(IPR, k, j,i+1) / pmb->phydro->w(IDN, k, j,i+1) / Rd;
            // du(IEN, k, j, i+1) -= dt*cool_coeff*sigma*_qur(temp)/2;
            break;                 
          }
        }
                //heating layer 2
        for (int i = pmb->ie; i>= pmb->is; --i){
          ...
          if (press >= 1e1){
            Real dz = pmb->pcoord->dx1f(i);
            du(IEN, k, j, i) += 0.10*Insolation(pmb,k,j,time) * dt / dz;
            break;
          }
        } 
      ...
      for (int i = is; i <= ie; ++i) {
        ...
        if (cosomega>0){
          if (current_time > 5.E9){
            ...
            Real fluxmax = eq_heat_flux + spinupflux/exp( current_time /5.E8);
