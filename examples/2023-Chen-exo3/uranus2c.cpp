...
static Real p0, Rd, cp, Ts, Rp, sponge_tau, grav, z_stra, eq_heat_flux,sday,syear, pi, dday, Omega, sigma;
Real current_time;
...
Real Insolation(MeshBlock *pmb,int k,int j,Real time){
  ...
  if (cosomega>0){
    Real fluxmax = eq_heat_flux + 10000./exp(time/1.E6);
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)
      for (int j = pmb->js; j <= pmb->je; ++j) {
          ...
          for (int i = pmb->ie-10; i<= pmb->ie; ++i){
            ...
            du(IEN, k, j, i) -= 0.25*sigma*_qur(temp) * dt / dz / 10; //emissivity of 0.25
          }
