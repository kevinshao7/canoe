      ...
      for (int j = pmb->js; j <= pmb->je; ++j) {
        ...
        for (int i = pmb->is; i<= pmb->ie; ++i){
          ...
          if (press <= 1e5){
            ...
            du(IEN, k, j, i) += 0.96*Insolation(pmb,k,j,time) * dt / dz;
            break;
          }
        }         
        //cooling layer 1 
        ...
        for (int i = pmb->ie; i>= pmb->is; --i){
          ...
          if (press >= 1e1){
            Real dz = pmb->pcoord->dx1f(i);
            du(IEN, k, j, i) += 0.04*Insolation(pmb,k,j,time) * dt / dz;
            break;
          }
        } 
  ...
  for (int k = ks; k <= ke; ++k)
    for (int j = js; j <= je; ++j) {
      ...
      for (; i <= ie; ++i) {
        ...
        // add noise                                                                                                              
        air.w[IVY] = 20* distribution(generator);
        air.w[IVZ] = 20* distribution(generator);
      }
    }
}
