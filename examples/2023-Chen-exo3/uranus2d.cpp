...
static Real p0, Rd, cp, Ts, Rp, grav, eq_heat_flux,sday,syear, pi, dday, Omega, sigma, emissivity,adtdz,cv,spinupflux;
Real current_time;
      ...
      for (int j = pmb->js; j <= pmb->je; ++j) {
          ...
          for (int i = pmb->is; i<= pmb->ie; ++i){
            if (pmb->phydro->w(IPR, k, j, i) <= 1e5){
              ...
              du(IEN, k, j, i) += Insolation(pmb,k,j,time) * dt / dz / 5;
            }
            if (heatinglayers >= 5){
              break;
            }
          }          // Energy taken away from the top at 0.0001 bar level    
          int coolinglayers=0;                                                                  
          for (int i = pmb->ie; i>=0; --i){
            if (pmb->phydro->w(IPR, k, j, i) >= 10){
              Real dz = pmb->pcoord->dx1f(i);
              du(IEN, k, j, i) -= 0.25*dt*(eq_heat_flux+ spinupflux/exp( time /5.E6))/dz/10;
              coolinglayers++;
