...
Real Insolation(MeshBlock *pmb,int k,int j,Real time){
  ...
  if (cosomega>0){
    Real fluxmax = eq_heat_flux + 1000./exp(time/1.E6);
    return fluxmax*cosomega;
  }
  else return 0;
}
Real DirchletPres(Real lat,Real temp){//input lat in radians
  Real isobarlevel = Rpol*Req/sqrt(_sqr(Rpol)*_sqr(cos(lat))+_sqr(Req)*_sqr(sin(lat))); //chord of ellipse
  Real boundarypress = p0*exp((isobarlevel-Rpol)*(-grav)/(Rd*temp)); //grav variable is negative
  return boundarypress;
}
...
void Forcing(MeshBlock *pmb, Real const time, Real const dt,
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)
      for (int j = pmb->js; j <= pmb->je; ++j) {
          // Energy input from bottom                                                                                                       
          Real dz = pmb->pcoord->dx1f(pmb->is);
          du(IEN, k, j, pmb->is) += Insolation(pmb,k,j,time) * dt / dz;
          // Energy taken away from the top, body cooling                                                                       
          for (int i = pmb->ie-10; i<= pmb->ie; ++i){
            dz = pmb->pcoord->dx1f(i);
          ...
          Real temp = pmb->phydro->w(IPR, k, j, pmb->is) / pmb->phydro->w(IDN, k, j, pmb->is) / Rd;
          pmb->phydro->w(IPR, k, j, pmb->is) = DirchletPres(lat,temp);
  }
}
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int k = ks; k <= ke; ++k)
    for (int j = js; j <= je; ++j) {
      ...
      Real lat, lon;
      pexo3->GetLatLon(&lat, &lon, k, j, is);
      air.w[IPR] = DirchletPres(lat,Ts);
      int i = is;
      for (; i <= ie; ++i) {
        AirParcelHelper::distribute_to_conserved(this, k, j, i, air);
        pthermo->Extrapolate(&air, pcoord->dx1f(i), "dry", grav, 0.002);
        // add noise                                                                                                              
  ...
  for (int k = ks; k <= ke; ++k)
    for (int j = js; j <= je; ++j)
      for (int i = is; i <= ie; ++i) {
        ...
        Real cosomega = sin(lat)*sin(latsun)+cos(lat)*cos(latsun)*cos(longsun-lon);
        if (cosomega>0){
          Real fluxmax = eq_heat_flux + 10000./exp(current_time/1.E6);
