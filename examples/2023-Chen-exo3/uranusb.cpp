...
static Real p0, Omega, Rd, cp, Ts, Rp, sponge_tau, grav, z_stra, heat_flux;
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k) {
    for (int j = pmb->js; j <= pmb->je; ++j) {
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        if (z > z_stra) {  // sponge layer at top and bottom
          Real tau = sponge_tau;
  ...
  for (int k = ks; k <= ke; ++k)
    for (int j = js; j <= je; ++j) {
      ...
      for (; i <= ie; ++i) {
        ...
        pthermo->Extrapolate(&air, pcoord->dx1f(i), "dry", grav, 0.000);
        // add noise
